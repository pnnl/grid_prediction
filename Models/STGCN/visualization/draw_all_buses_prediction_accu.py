...
def draw_all_buses_prediction_accu(y, y_, x_stats, p_start_t, load_path):
    ...
    cores_ground_value = ground_value[:, p_start_t:p_start_t + predict_value.shape[1]]
    core_ground = ground_value[:,500:p_start_t+predict_value.shape[1]]
    print(core_ground.shape)
    ...
    x_1 = np.arange(0, 500+predict_value.shape[1], 1)
    for i in range(ground_value.shape[0]):
        plt.plot(x_1, core_ground[i], color='blue', label='Ground truth')
        plt.plot(x_1[500:500+predict_value.shape[1]], predict_value[i], '--', linewidth=2, color='red', label='Predicted using STGCN')
